...
GourceSettings::GourceSettings() {
    ...
    arg_types["file-size-scale-interpolation"] = "float";
...
void GourceSettings::setGourceDefaults() {
    ...
    file_size_scale_interpolation = 1.0f;
    file_scale = 1.0f;
...
void GourceSettings::importGourceSettings(ConfFile& conffile, ConfSection* gource_settings) {
    ...
    bool has_file_size_scale_interpolation = false;

    if(gource_settings->getBool("scale-by-file-size")) {
        scale_by_file_size = true;
    }

    if((entry = gource_settings->getEntry("file-size-scale-interpolation")) != 0) {

        if(!entry->hasValue()) conffile.entryException(entry, "specify file-size-scale-interpolation (float)");

        file_size_scale_interpolation = entry->getFloat();

        if(file_size_scale_interpolation < 0.0f || file_size_scale_interpolation > 2.0f) {
            conffile.entryException(entry, "file-size-scale-interpolation outside of range 0.0 - 2.0 (inclusive)");
        }

        has_file_size_scale_interpolation = true;
    }

    if(has_file_size_scale_interpolation) {
        scale_by_file_size = file_size_scale_interpolation > 0.0f;
    }
